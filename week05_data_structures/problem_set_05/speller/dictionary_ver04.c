...
// Constants for additional "refinements" for hashing
const int LOWER_BOUND = 4;
const int UPPER_BOUND = 16;
const int BASE = 26 * (LOWER_BOUND + (LENGTH + 1 - UPPER_BOUND));

// TODO: Choose number of buckets in hash table
const unsigned int N = BASE + LENGTH * LENGTH;
...
unsigned int hash(const char *word)
{
    ...
    {
        hash = BASE + letter * (toupper(word[1]) - 'A');
    }
    else
    {
        hash = letter * length;
...
bool load(const char *dictionary)
{
    ...
    // Release memory ?
    // Free word since it was copied in add_to_table()
    free(word);
